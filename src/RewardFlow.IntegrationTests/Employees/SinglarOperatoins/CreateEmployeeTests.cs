...
public class CreateEmployeeTests(TestWebApplicationFactory factory) : BaseEmployeeTestFixture(factory), IAsyncLifetime
{
    ...
    {
        // Arrange
        await _dbUtility.InsertRangeAsync(TestDataGenerator.Employee.ForProperty(e=>e.TenantId, _userClient.User.TenantId).Generate(20));
    ...
    public async Task CreateEmployee_WithSameData_DifferentUsers_ShouldReturnCreated()
    {
        ...
        // Assert
        await AssertEmployeeCreationSuccess(response1, request1, UserClient1.User.Id);
        await AssertEmployeeCreationSuccess(response2, request2, UserClient2.User.Id);
    }
    /// <summary>
    ...
    private async Task AssertEmployeeCreationSuccess(HttpResponseMessage response, CreateEmployee.Request request, int? userId = null)
    {
        response.StatusCode.Should().Be(HttpStatusCode.Created);
        var employee = await response.Content.ReadFromJsonAsync<EmployeeDto>();
        employee.Should().NotBeNull();
        employee.Id.Should().NotBe(0);
        employee.Name.Should().Be(request.Name);
        employee.NationalNumber.Should().Be(request.NationalNumber);

        employee.CreatedAt.Should().BeOnOrBefore(DateTime.UtcNow.AddSeconds(1));
        ...
        var tokensExist = await _dbUtility.Query<EmployeeNameToken>()
            .AnyAsync(t => t.EmployeeId == employee.Id && t.UserId == (userId ?? _userClient.User.Id));
